...
import { ProductSponsoring, ProductTile, ProductTileLayout, ProductTileProps } from '@/components/Organisms/ProductTile'
...
export const ProductCarousel: React.FC<ScrollProductCarouselProps> = ({
  ...
  const carouselOrientation = playlistLayout === ProductTileLayout.gridItem ? 'portrait' : 'landscape'
  const isGridLayout = playlistLayout === ProductTileLayout.gridItem

  const renderProduct =
    (productsPerSlide: number) =>
    (product: Product | Partial<Product>, index: number): JSX.Element => {
  ...
  const renderProducts = () => {
    if (!promoted) return [...products.map(renderProduct(productsPerSlideWithHook)), <div key="right-spacer" />]
  ...
  return (
    ...
    >
      {sponsoringDetails ? <ProductSponsoring layout="carousel" sponsoringDetails={sponsoringDetails} /> : null}
      <ScrollSnapSlider
