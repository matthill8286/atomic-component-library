...
export const ProductTileCompactElement: React.FC<ProductTileCompactChildProps> = ({
  ...
  const productInformation = !loading ? (
    <>
      <CopyText limitLines={2} tag="div" display="flex" color={'grey6'} fontSize="md" margin="sm 0 0">
        {title}
      </CopyText>
      {!isMobile ? (
        <FlexBox display="inline-flex">
          {provider ? (
            <StyledInformationTypo display="inline-flex" tag="div" color={'grey4'} fontSize="xs" padding="0 sm 0 0">
              {provider}
            </StyledInformationTypo>
          ) : null}
          {type ? (
            <StyledInformationTypo display="inline-flex" tag="div" color={'grey4'} fontSize="xs" padding="0 sm">
              {type}
            </StyledInformationTypo>
          ) : null}
          {competency ? (
  ...
  return (
    <StyledProductTileCompactElementWrapper {...props}>
      ...
      <StyledRightIconsWrapper>
        {showInteractions && !loading && <StyledIconsWrapper>{renderRightIcons()}</StyledIconsWrapper>}
        {children}
      </StyledRightIconsWrapper>
    </StyledProductTileCompactElementWrapper>
  )
