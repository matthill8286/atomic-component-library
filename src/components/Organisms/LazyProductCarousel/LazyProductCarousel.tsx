...
export const LazyProductCarousel: React.FC<ProductCarouselProps> = ({
  ...
  const productsPerSlideWithHook =
    breakpoint >= breakpoints.lg ? largeSlides : breakpoint >= breakpoints.md ? mediumSlides : smallSlides

  const currentConfig =
    breakpoint >= breakpoints.lg ? largeConfig : breakpoint >= breakpoints.md ? mediumConfig : smallConfig

  const getLeftOverflow = (config: CarouselConfigProps): boolean => config.hideLeftOverflow ?? hideLeftOverflow
  ...
  const handleSlideChange = (productsPerSlide: number) => (nextSlideIndex: number) => (products: Product[]) => {
    setIsDragging(false)
  ...
  const handleProductClick =
    ...
    (e: React.MouseEvent<HTMLElement>, product: Product, index: number, pageNumber: number) => {
      if (isDragging) {
        e.preventDefault()
      } else {
    ...
    <ClippingWrapper hideLeft={getLeftOverflow(currentConfig)}>
      ...
      <SimpleCarousel
        ...
        sliderSettings={{
          ...
          afterChange: (current: number) => handleSlideChange(productsPerSlideWithHook)(current)(products),
