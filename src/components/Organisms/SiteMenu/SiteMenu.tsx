...
import { TabBar, TabData, TabId } from '@/components/Molecules/TabBar'
...
import { Color, Elevation, ThemeColors } from '@/types/theme'
...
const StyledSiteMenuCard = styled(Card)<{
  ...
  ${StyledCard} {
    ${({ enableSemanticTheme, theme, surfaceColor }) =>
      enableSemanticTheme &&
      css`
        background-color: ${theme.color[surfaceColor as keyof typeof theme.color]};
...
export const SiteMenu: FC<SiteMenuProps> = memo(
  ({
    isFixed,
    menuItems,
    activeItemId,
    ...
    const handleChange = (id: TabId, data: TabData | undefined) => {
