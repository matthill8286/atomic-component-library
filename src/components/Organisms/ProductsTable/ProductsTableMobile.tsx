...
export const ProductsTableMobile: React.FC<ProductsTableProps> = ({ tableHeading, tableContent, getProductUrl }) => {
              ...
              return (
                <TableRow key={'feature-' + index} collapsible={false}>
                  ...
                  {productRow.map((feature: TableRowDataProps, index) => {
                    return (
                      <TableCell key={feature.text + index} borderDirection="right" collapsible={false}>
  ...
  const { featureCells, contentCells } = useMemo(() => transposedContent(tableContent), [tableContent])
